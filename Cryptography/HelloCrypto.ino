...
*/
constexpr char masterKey[] PROGMEM = "hellomessage"; // Use 8 random characters or more
...
void loop() {
  ...
  String exampleData = F("Hello_World_This_is_an_example_data");
  Serial.println(String(F("This is our example data: ")) + exampleData);

  uint8_t resultArray[SHA256::NATURAL_LENGTH] { 0 }; //unsigned variable array
  uint8_t derivedKey[ENCRYPTION_KEY_LENGTH] { 0 }; //unsigned variable array
  ...
  String dataToEncrypt = F("This is an Authenticated Encryption message\n");
  ...
  ChaCha20Poly1305::encrypt(dataToEncrypt.begin(), dataToEncrypt.length(), derivedKey, &encryptionCounter, sizeof encryptionCounter, resultingNonce, resultingTag); //Poly1305 Authenticated encryption with additional data (AEAD) algorithm
  Serial.println(String(F("Encrypted data: ")) + dataToEncrypt);

  //checking on a boolean variable if a data is succeeded 
